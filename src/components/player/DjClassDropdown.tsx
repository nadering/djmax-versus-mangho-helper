...
interface DjClassProps {
  name: string;
  isSub: boolean;
  subContent?: DjClassProps[];
}

const DJ_CLASS_LIST: DjClassProps[] = [
...
interface DjClassDropdownProps {
  button: string;
}

/** DJ CLASS 정보를 설정하는 드랍다운 */
const DjClassDropdown = memo(({ button }: DjClassDropdownProps) => {
  ...
  return (
    <DropdownMenu.Root>
      <DropdownMenu.Trigger asChild>
        ...
        >
          <span
            className={`absolute top-[50%] left-0 right-0 translate-y-[-50%] mx-auto ${value === button ? "text-gray-400" : "font-medium text-gray-800"}`}
          >
            {value}
          </span>
          <span
              ...
              <DropdownMenu.Sub key={item.name}>
                ...
                >
                  {item.subContent.map((subItem) => (
                    <DropdownMenu.Item
                      key={subItem.name}
                      onSelect={() => setValue(`${item.name}${subItem.name}`)}
