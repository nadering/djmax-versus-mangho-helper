...
import { DJ_CLASS_LIST } from "@/constants/DjClass";
...
const DjClassDropdown = memo(
  ({ id, button, value, onDjClassSelect }: DjClassDropdownProps) => {
    ...
    const itemClass =
      "px-3 py-1.5 text-sm text-gray-800 rounded-md cursor-pointer hover:bg-gray-100 active:bg-gray-200";
    ...
    return (
      <DropdownMenu.Root>
        ...
        >
            ...
            if (item.isSub && item.subContent) {
              return (
                <DropdownMenu.Sub key={item.name}>
                  <DropdownMenu.SubTrigger className={`relative ${itemClass}`}>
                    {item.name}
                  ...
                  <DropdownMenu.SubContent
                    sideOffset={4}
                    className="bg-white shadow-md rounded-lg border w-12 p-1 z-50"
                  >
          ...
          <div
            className="h-0 border-t-1 border-gray-400 my-2"
            role="separator"
          />
          <DropdownMenu.Item
            onSelect={() =>
